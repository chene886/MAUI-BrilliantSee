...
namespace BrilliantSee.Services
{
    public class DBService
    {
        ...
        /// 数据库文件名
        /// </summary>
        private const string _dbFile = "BrilliantSee.db3";
        ...
        private async Task InitAsync()
        {
            ...
            var Share = "我正在使用BrilliantSee看漫画，资源丰富、功能强大，快来下载体验吧！下载地址：https://www.123pan.com/s/6cnjjv-6njBv.html";
        ...
        /// <returns></returns>
        public async Task<bool> IsComicExistAsync(Obj comic, DBObjCategory category)
        {
            return await _db.Table<DBObj>().Where(i => i.Url == comic.Url && i.Category == category).CountAsync() > 0;
        }
        ...
        {
            var dbObj = obj.CreateDBObjFromObj(obj);
            dbObj.Category = category;
            if (_db.Table<DBObj>().Where(i => i.Url == obj.Url && i.Category == category).CountAsync().Result > 0)
            {
                await this.DeleteObjAsync(obj, category);
            }
            return await _db.InsertAsync(dbObj);
        }
        ...
        /// <returns></returns>
        public async Task<int> UpdateComicAsync(Obj comic)
        {
            var dbComic = comic.CreateDBObjFromObj(comic);
            var existingComic = await _db.Table<DBObj>()
        ...
        /// 删除漫画
        /// </summary>
        /// <param name="obj">需要删除的漫画</param>
        /// <param name="category">漫画的类型</param>
        /// <returns></returns>
        public async Task<int> DeleteObjAsync(Obj obj, DBObjCategory category)
        {
            return await _db.Table<DBObj>().DeleteAsync(i => i.Url == obj.Url && i.Category == category);
        }
